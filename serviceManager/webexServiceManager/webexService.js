...
const consultantMap = {"Consultant1": "Y2lzY29zcGFyazovL3VzL1BFT1BMRS8wZjk5YTMzMi1mNmFlLTQxMDUtYTI5OS1hZTA4ZGExYTFlNGQ", "Consultant4": "Y2lzY29zcGFyazovL3VzL1BFT1BMRS81ODJkZmM3NC1lZWZmLTRiY2ItYjVmOS1mM2Q0OWRkOTZhNzU"};
...
exports.setupMeeting = function (bot, message, consultantName, id, userName) {
      ...
      if (err) {
        ...
        client.createRoom(roomTitle, function(err, resp, body) {
          ...
          if(!err && resp.statusCode === 200) {
            ...
            let userId;
            let userEmail;
            User.findOne({"UserId": message.user}, function(err, result) {
              if (err) {
                ...
                client.addUSerToRoom(roomId, userId, false, function(err, resp, body) {
                  ...
                  if(!err && resp.statusCode === 200) {
                    ...
                    client.addUSerToRoom(roomId, consultantId, false, function(err, resp, body) {
                      ...
                      if (!err && resp.statusCode === 200) {
                        Consultant.findOne({"webexId": consultantId}, function(err, result) {
                          if (err) {
  ...
  var client = {
    ...
    createRoom: function (roomTitle, onResponse) {
      ...
      needle.post("https://api.ciscospark.com/v1/rooms", data, {headers: headers, json:true}, onResponse);
    },
    ...
    addUSerToRoom: function(roomId, userId, isModeratorFlag, onResponse) {
      ...
      needle.post("https://api.ciscospark.com/v1/memberships", data, {headers: headers, json: true}, onResponse);
    }
  }
